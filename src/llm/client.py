...
def generate_recommendation(
    ...
    prompt = (
        ...
        "avoid.\n"
        "- same_team_stack_penalty: fires only for a WR/TE who'd share an "
        "NFL team's target pool with a pass-catcher already on your roster "
        "(WR+WR is the harshest penalty, WR+TE milder) -- NOT applied to QB "
        "stacks (QB+same-team pass-catcher is a deliberate, desired "
        "correlation strategy, never a penalty) or to any RB pairing (not a "
        "real target-competition effect). Like reach_penalty, it's a real "
        "but not absolute deterrent already folded into composite_score.\n"
        "- strategic_profile: a short human-readable tag (e.g. 'Dead Zone "
        "Volume RB', 'Market Fall', 'High Ceiling', 'Same-Team Stack Risk') "
        "for context.\n\n"
